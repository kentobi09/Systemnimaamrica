    ...
    <table class="table table-hover" data-table-type="handson">
        ...
        <tbody id="applicant-table-body">
            ...
            <?php foreach ($applicant_record as $applicant) : ?>
                ...
                <?php $handsonFound = false; ?>
                <?php foreach ($applicanthandson as $handson) : ?>
                    <?php if ($handson['applicantID'] === $applicant['applicantID']) : ?>
                        <?php $handsonFound = true; ?>
...
>
    <i class="bi bi-pencil-square text-white"></i>
                ...
                <?php endforeach; ?>
                <?php if (!$handsonFound) : ?>
                    <!-- If no handson record found, display the applicant record without handson details -->
                    <tr data-applicant-id="<?= $applicant['applicantID'] ?>">
                        <td class="delete-column">
                            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                <button onclick="deletexid(<?php echo $applicant['applicantID'] ?>)" type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteApplicantModal" data-applicantid="<?php echo $applicant['applicantID']; ?>">
                                ...
                                <button class="btn btn-secondary edit-applicant-btn" type="button" data-toggle="modal" data-target="#editApplicantModal" data-id="<?php echo $applicant['applicantID']; ?>" data-fname="<?php echo htmlspecialchars($applicant['firstname']); ?>" data-mname="<?php echo htmlspecialchars($applicant['middlename']); ?>" data-lname="<?php echo htmlspecialchars($applicant['lastname']); ?>" data-sex="<?php echo htmlspecialchars($applicant['sex']); ?>" data-province="<?php echo htmlspecialchars($applicant['province']); ?>" data-contact="<?php echo htmlspecialchars($applicant['contact_number']); ?>" data-email="<?php echo htmlspecialchars($applicant['email_address']); ?>" data-notification="<?php echo htmlspecialchars($applicant['date_of_notification']); ?>" data-exam-date="<?php echo htmlspecialchars($applicant['date_of_examination']); ?>" data-exam-venue="<?php echo htmlspecialchars($applicant['exam_venue']); ?>" data-proctor="<?php echo htmlspecialchars($applicant['proctor']); ?>" data-status="<?php echo htmlspecialchars($applicant['status']); ?>" <?php if (isset($scores[$applicant['applicantID']])) : ?> <?php $score = $scores[$applicant['applicantID']]; ?> data-score1="<?php echo htmlspecialchars($score['score1']); ?>" data-score2="<?php echo htmlspecialchars($score['score2']); ?>" data-score3="<?php echo htmlspecialchars($score['score3']); ?>" data-total="<?php echo htmlspecialchars($score['total']); ?>" data-handson="<?php echo htmlspecialchars($score['hands_on']); ?>" <?php else : ?> data-score1="" data-score2="" data-score3="" data-total="" <?php endif; ?> data-application-form="<?php echo !empty($applicant['application_form']); ?>">
                                    <i class="bi bi-pencil-square text-white"></i>
                                </button>
                            </div>
                        </td>
