...
try {
    ...
    if(isset($_POST['delete_applicant'])){
        ...
        // Get the applicant's names before deleting
        ...
        if($result){
            ...
            $deletedapplicant = "Deleted applicant record but could not retrieve names.";
        }
        ...
        // Delete related hands-on records in applicanthandson
        $deleteHandsOnSql = "DELETE FROM applicanthandson WHERE applicantID = :applicantID";
        $deleteHandsOnStmt = $conn->prepare($deleteHandsOnSql);
        $deleteHandsOnStmt->bindParam(':applicantID', $applicantID, PDO::PARAM_INT);
        $deleteHandsOnStmt->execute();
